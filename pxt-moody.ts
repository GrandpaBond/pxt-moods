...
// ---Create Built-in FlexFXs----
    ...
    */
    ...
    export function hum(repeat = 12, strength = 180, duration = 4000) {
        ...
        for (let index = 0; index < repeat; index++) {
            ...
            if ((span > ave) || (skip)) {
                // mostly "Dum"...
                flexFX.playFlexFX(MoodSound.DOO.toString(), pitch, strength, span);
                basic.pause(gap);
                skip = false;
            } else {
                // .. with occasional short, higher-pitched "Di" (unrepeated)
                flexFX.playFlexFX(MoodSound.DOO.toString(), 1.33 * pitch, strength, 0.3 * ave);
                basic.pause(gap / 2);
                skip = true;
            }
        }
    ...
    */
    ...
    export function grumble(repeat = 5, strength = 180, duration = 4000) {
        ...
        for (let index = 0; index < repeat; index++) {
            ...
            if (span > 0.8 * ave) {
                flexFX.playFlexFX(MoodSound.DUH.toString(), randint(150, 300), strength, 0.5 * span);
            } else {
                flexFX.playFlexFX(MoodSound.UHOH.toString(), randint(100, 200), 1.2 * strength, 2 * span);
            }
            pause(gap);
        }
        quiet = true;
    ...
    */
    ...
    export function giggle(repeat = 10, strength = 200, duration = 2000) {
        ...
        for (let index = 0; index < repeat; index++) {
            let span = randint(0.4 * ave, 1.8 * ave);
            flexFX.playFlexFX(MoodSound.LAUGH.toString(), pitch, strength, span);
            pitch = 0.95 * pitch;
            basic.pause(gap);
        }
        quiet = true;
    ...
    */
    ...
    export function whistle(repeat = 12, strength = 180, duration = 2500) {
        ...
        for (let index = 0; index < repeat; index++) {
            let span = randint(0.4 * ave, 1.8 * ave);
            flexFX.playFlexFX(MoodSound.TWEET.toString(), randint(800, 2000), strength, span);
            basic.pause(gap);
        }
        quiet = true;
    }
    ...
    */
    ...
    export function snore(repeat = 6, strength = 150, duration = 10000) {
        ...
        for (let index = 0; index < repeat; index++) {
            ...
            flexFX.playFlexFX(MoodSound.SNORE.toString(), 1, 80, 0.3 * span);
            pause(0.1 * ave);
            flexFX.playFlexFX(MoodSound.SNORE.toString(), 1, 150, 0.4 * span);
            pause(0.3 * ave);
        }
        quiet = true;
    }
    ...
    */
    ...
    export function whimper(repeat = 8, strength = 150, duration = 5000) {
        if (quiet) {
            ...
            let gap = 0.5 * ave;
            for (let index = 0; index < repeat; index++) {
                flexFX.playFlexFX(MoodSound.MOAN.toString(), randint(350, 550), strength, randint(0.4 * ave, 1.4 * ave));
                basic.pause(gap);
            }
            quiet = true;
        }
    ...
    export function cry(repeat = 8, strength = 200, duration = 3500) {
        if (quiet) {
            ...
            for (let index = 0; index < repeat; index++) {
                ...
                if (span > ave) {
                    flexFX.playFlexFX(MoodSound.MOAN.toString(), randint(350, 550), 1.25 * strength, 0.7 * span);
                } else {
                    flexFX.playFlexFX(MoodSound.WAAH.toString(), randint(350, 450), 0.25 * strength, 1.3 * span);
                }
                basic.pause(gap);
            }
            quiet = true;
    ...
    */
    ...
    export function shout(repeat = 5, strength = 250, duration = 2500) {
        if (quiet) {
            ...
            let gap = 0.3 * ave;
            for (let index = 0; index < repeat; index++) {
                flexFX.playFlexFX(MoodSound.GROWL.toString(), randint(320, 450), strength, randint(0.5 * ave, 1.3 * ave));
                basic.pause(gap);
            }
            quiet = true;
        }
    ...
    export function doMood(mood: number) {
        switch (mood) {
            case 1: hum();
                break;
            case 2: grumble();
    ...
    export function doSound(sound: number) {
        switch (sound) {
            case 1: flexFX.playFlexFX(MoodSound.TWEET.toString(), 800, 200, 400);
                break;
            case 2: flexFX.playFlexFX(MoodSound.LAUGH.toString(), 400, 200, 400);
                break;
            case 3: flexFX.playFlexFX(MoodSound.SNORE.toString(), 1, 200, 400);
                break;
            case 4: flexFX.playFlexFX(MoodSound.DOO.toString(), 500, 200, 300);
                break;
            case 5: flexFX.playFlexFX(MoodSound.QUERY.toString(), 400, 200, 700);
                break;
            case 6: flexFX.playFlexFX(MoodSound.UHOH.toString(), 350, 200, 650);
                break;
            case 7: flexFX.playFlexFX(MoodSound.MOAN.toString(), 500, 200, 700);
                break;
            case 8: flexFX.playFlexFX(MoodSound.DUH.toString(), 300, 200, 500);
                break;
            case 9: flexFX.playFlexFX(MoodSound.WAAH.toString(), 600, 200, 1100);
                break;
            case 10: flexFX.playFlexFX(MoodSound.GROWL.toString(), 250, 200, 700);
        }
        basic.pause(1000)
    }
}
...
input.onButtonPressed(Button.B, function () {
    if (testMoods) {
        moody.doMood(choice + 1);
    } else {
        moody.doSound(choice + 1);
    }
})
